...
C_DEFINES = ('\n/*\n * Defines\n */\n\n'
        'const int OC_NO_ERROR = 0;\n'
        'const int OC_FORCE_ERROR = -1;\n'
        'const int OC_POINTER_IS_NULL = -2;\n'
...
def write_c_header(library, output):
    ...
    output.write('/*\n * opencmiss.h. This file is automatically generated '
        ...
        '    } \\\n'
        '    else if(E == OC_ERROR_CONVERTING_POINTER) { \\\n'
        '      fprintf(stderr,"Error: %s: Error converting pointer.\\n",(S)); \\\n'
        '    } \\\n'
        '    else if(E == OC_POINTER_IS_NULL) { \\\n'
        '      fprintf(stderr,"Error: %s: Pointer is null.\\n",(S)); \\\n'
        '    } \\\n'
        '    else if(E == OC_POINTER_NOT_NULL) { \\\n'
        '      fprintf(stderr,"Error: %s: Pointer is not null.\\n",(S)); \\\n'
        '    } \\\n'
        '    else if(E == OC_COULD_NOT_ALLOCATE_POINTER) { \\\n'
...
def write_c_f90(library, output):
    ...
    output.write('!\n! opencmiss_c.F90. This file is automatically generated '
        ...
        '  INTEGER(C_INT), PARAMETER :: OC_POINTER_NOT_NULL = -3\n'
        '  INTEGER(C_INT), PARAMETER :: OC_COULD_NOT_ALLOCATE_POINTER = -4\n'
        '  INTEGER(C_INT), PARAMETER :: OC_ERROR_CONVERTING_POINTER = -5\n\n')
...
def subroutine_to_c_f90(subroutine):
    ...
    content.append('')
    content.append('%s = OC_NO_ERROR' % c_f90_name)
...
def parameter_conversion(parameter):
    ...
    if parameter.var_type == Parameter.CUSTOM_TYPE:
        ...
        # If we're in a OC...TypeInitialise routine, then objects get
        ...
        if parameter.routine.name.endswith('_Initialise'):
            ...
            pre_call.extend(('IF(C_ASSOCIATED(%sPtr)) THEN' % parameter.name,
                '%s = OC_POINTER_NOT_NULL' % routine_c_f90_name,
                'ELSE',
                'NULLIFY(%s)' % parameter.name,
                'ALLOCATE(%s, STAT = Err)' % parameter.name,
                'IF(Err /= 0) THEN',
                '%s = OC_COULD_NOT_ALLOCATE_POINTER' % routine_c_f90_name,
                'ELSE'))
            ...
            post_call.extend(('DEALLOCATE(%s)' % parameter.name,
                ...
                'ELSE',
                '%s = OC_ERROR_CONVERTING_POINTER' % routine_c_f90_name,
                'ENDIF',
                'ELSE',
                '%s = OC_POINTER_IS_NULL' % routine_c_f90_name,
                'ENDIF'))
        else:
            if parameter.array_dims > 0:
                ...
                if parameter.intent == 'IN':
                    # Passing an array of OC Types to Fortran
                    pre_call.append('CALL %ssCopy(%s,%sSize,%sPtr,%s)' %
                            (parameter.type_name, parameter.name,
                            parameter.name, parameter.name,
                            routine_c_f90_name))
                else:
                    # Getting an array of OC Types from Fortran and
