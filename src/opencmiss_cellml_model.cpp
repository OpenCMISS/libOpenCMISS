...
#include "opencmiss_cellml_model_config.h"
  ...
  mURL(url)
{
  ...
  std::cout << "Creating CellMLModel from the URL: " << url << std::endl;
  ...
  if (numberOfValidationIssues != 0) {
    std::cout << "The validator has found " << numberOfValidationIssues << " issues!" << std::endl;
...
int CellMLModel::getInitialValue(const char* name, double* value)
{
  ...
  if (!model)
    {
      std::cerr << "CellMLModel::getInitialValue -- missing model?" << std::endl;
      return -2;
    }
  RETURN_INTO_OBJREF(var,iface::cellml_api::CellMLVariable,findLocalVariable(model,mCevas,name));
  if (!var)
    {
      std::cerr << "CellMLModel::getInitialValue -- unable to find named variable: " << name << std::endl;
      return -3;
    }
  ...
  if (iv != L"")
    {
      ...
      if (end == NULL || *end != 0)
	{
	  std::wcerr << "CellMLModel::getInitialValue -- variable " << name << " has non-numeric initial value: "
		     << iv.c_str() << std::endl;
	  code = -5;
	}
      else
    ...
    {
      std::cerr << "CellMLModel::getInitialValue -- variable " << name << " has no initial value." << std::endl;
      code = -4;
    }
#endif
  return code;
...
int CellMLModel::getInitialValueByIndex(const int type, const int index, double* value)
{
#ifdef CELLML_USE_CSIM
  switch (type)
    {
  ...
  if (i == mInitialValues.end())
    {
      std::cerr << "CellMLModel::getInitialValueByIndex -- unable to find variable: " << index
...
int CellMLModel::getVariableType(const char* name,int* type)
{
#ifdef CELLML_USE_CSIM
  unsigned char vt = model->getVariableType(name);
  if (vt == csim::UndefinedType)
    {
      std::cerr << "CellMLModel::getVariableType -- undefined type" << std::endl;
      return -2;
    }
  ...
  if (vt & csim::StateType) *type = StateType;
    ...
    {
      std::cerr << "CellMLModel::getVariableType -- unkown type found? (" << vt << ")" << std::endl;
      return -3;
    }
  return 0;
#else
  ...
  if (!model && !mCodeInformation && !mAnnotations)
    {
      std::cerr << "CellMLModel::getVariableType -- missing model?" << std::endl;
      return -2;
    }
  ...
  if (!var)
    {
      std::cerr << "CellMLModel::getVariableType -- unable to find named variable: " << name << std::endl;
      return -3;
    }
  ...
  if (i == mVariableTypes.end())
    {
      std::cerr << "CellMLModel::getVariableType -- unable to find named variable's type: " << name << std::endl;
      return -4;
    }
...
int CellMLModel::getVariableIndex(const char* name,int* index)
{
#ifdef CELLML_USE_CSIM
  ...
  std::cerr << "CellMLModel::getVariableIndex -- unable to get the type of variable " << name << std::endl;
  return -1;
#else
  iface::cellml_api::Model* model = static_cast<iface::cellml_api::Model*>(mModel);
  if (!model && !mCodeInformation && !mAnnotations)
    {
      std::cerr << "CellMLModel::getVariableIndex -- missing model?" << std::endl;
      return -2;
    }
  ...
  if (!var)
    {
      std::cerr << "CellMLModel::getVariableIndex -- unable to find named variable: " << name << std::endl;
      return -3;
    }
  ...
  if (i == mVariableIndices.end())
    {
      std::cerr << "CellMLModel::getVariableIndex -- unable to find named variable's type: " << name << std::endl;
      return -4;
    }
...
{
#ifdef CELLML_USE_CSIM
  std::cerr << "CellMLModel::flagVariable -- not implemented with CSim" << std::endl;
  return -2;
#else
  if (!mCodeInformation)
    {
      std::cerr << "CellMLModel::flagVariable -- missing model?" << std::endl;
      return -1;
    }
  ...
  if (currentAnnotation != mVariableTypes.end())
	...
	{
	  std::cerr << "CellMLModel::flagVariable -- variable already flagged something else: "
		    << name << std::endl;
	  return -5;
	}
    }
  ...
  if (!ct)
    {
      std::cerr << "CellMLModel::flagVariable -- unable get computation target for the source of variable: "
		<< name << std::endl;
      return -5;
    }
  ...
  if (compatible)
    ...
    {
      std::cerr << "CellMLModel::flagVariable -- computation target for variable: "
...
int CellMLModel::instantiate()
{
  ...
  std::cerr << "CellMLModel::instantiate: unable to instantiate the CSim model: " << code << std::endl;
  return code;
#else
  int code = -1;
  int systemCode = 0;
  ...
  if (codeString.length() > 1)
      ...
      if (mkdtemp(templ))
	{
      ...
      systemCode = system(compileCommand);
      std::cout << "system command result: \"" << systemCode << "\"" << std::endl;
      std::cout << "system command error: \"" << strerror(systemCode) << "\"" << std::endl;
      if (systemCode == 0)
	{
	  mDsoFileExists = true;
							    ...
							    void* _cevas, const char* name)
{
  if (!_cevas)
    {
      std::cerr << "CellMLModel::findLocalVariable -- missing CeVAS object?" << std::endl;
      return NULL;
    }
  ...
  if (!component)
    {
      std::cerr << "CellMLModel::findLocalVariable -- unable to find component: " << cv.first.c_str() << std::endl;
      return NULL;
    }
  ...
  if (!variable)
    {
      std::cerr << "CellMLModel::findLocalVariable -- unable to find variable: " << cv.first.c_str() << " / "
  ...
  if (!v)
    {
      std::cerr << "CellMLModel::findLocalVariable -- unable get source variable for variable: "
		<< cv.first.c_str() << " / " << cv.second.c_str() << std::endl;
      return NULL;
    }
  return v;
...
#ifndef CELLML_USE_CSIM
std::wstring 
CellMLModel::getModelAsCCode(void* _model,void* _cevas,void* _annotations)
{
