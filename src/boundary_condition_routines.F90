...
999 ERRORSEXITS("BoundaryConditions_AddNodeVariable",err,error)
  ...
  SUBROUTINE BoundaryConditions_CalculateAnalyticDOFValue(boundaryConditions,fieldVariable,versionNumber,derivativeNumber, &
    & nodeNumber,componentNumber,numberOfDimensions,analyticValue,gradientAnalyticValue,hessianAnalyticValue,position,normal, &
    & tangents,analyticDOFValue,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error !<The error string
    !Local Variables
    INTEGER(INTG) :: componentIdx1,componentIdx2,derivativeGlobalIndex,variableType
    REAL(DP) :: dofValue
    TYPE(VARYING_STRING) :: localError
...
!!TODO: not implemented
      ...
      CASE DEFAULT
        localError="The global derivative index of "//TRIM(NumberToVString(derivativeGlobalIndex,"*",err,error))// &
          & " for derivative index "//TRIM(NumberToVString(derivativeNumber,"*",err,error))//" of local node number "// &
          & TRIM(NumberToVString(nodeNumber,"*",err,error))//" of component number "// &
...
!!TODO: not implemented
    ...
    analyticDOFValue=dofValue
    
    EXITS("BoundaryConditions_CalculateAnalyticDOFValue")
    RETURN
999 ERRORSEXITS("BoundaryConditions_CalculateAnalyticDOFValue",err,error)
    RETURN 1
    
  END SUBROUTINE BoundaryConditions_CalculateAnalyticDOFValue
