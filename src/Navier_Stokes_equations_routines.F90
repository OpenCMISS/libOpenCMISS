...
999 ERRORSEXITS("NavierStokes_ProblemSetup",err,error)
  ...
  SUBROUTINE NavierStokes_FiniteElementResidualEvaluate(equationsSet,elementNumber,err,error,*)
    ...
    REAL(DP) :: aDeriv,a0Deriv,a0Param,alpha,area,AUpwind,aValue,beta,C1,columnPhi,crossStress,currentTime,dColumnPhidXi(3), &
      & deltaTime,doubleDotG,dPhidX(3),dRowPhidXi(3),dXidX(3,3),eDeriv,elementInverse,eParam,g0Param,gaussWeight, &
    ...
    LOGICAL :: ALE,firstAssemblyDamping,firstAssemblyRHS,firstAssemblyStiffness,linearElement,RBSStabilisation,RBVMStabilisation, &
      & stabilisation,static,update,updateDamping,updateMatrices,updateResidual,updateRHS,updateStiffness
...
999 ERRORSEXITS("NavierStokes_FiniteElementResidualEvaluate",err,error)
  ...
  SUBROUTINE NavierStokes_FiniteElementJacobianEvaluate(equationsSet,elementNumber,err,error,*)
    ...
    INTEGER(INTG) :: columnComponentIdx,columnElementDOFIdx,columnElementParameterIdx,columnMeshComponent,componentIdx, &
      ...
      & stabilisationType,velocityInterpolationOrder(3),versionIdx,xiIdx,xiIdx1,xiIdx2
    ...
    LOGICAL  :: ALE,linearElement,RBSStabilisation,RBVMStabilisation,stabilisation,static,updateJacobian
    TYPE(BasisType), POINTER :: dependentBasis,geometricBasis,independentBasis
    TYPE(BasisPtrType) :: columnBasis(4),rowBasis(4)
    TYPE(DecompositionType), POINTER :: dependentDecomposition,geometricDecomposition,independentDecomposition
    TYPE(DomainType), POINTER :: columnDomain,dependentDomain,geometricDomain,independentDomain,rowDomain
...
999 ERRORSEXITS("NavierStokes_FiniteElementJacobianEvaluate",err,error)
  ...
  SUBROUTINE NavierStokes_PostSolve(solver,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error !<The error string
    !Local Variables
    INTEGER(INTG) :: equationsSetIdx,esSpecification(3),inputIteration,iterationNumber,loopLevel,loopType, &
...
999 ERRORSEXITS("NavierStokes_PostSolve",err,error)
  ...
  SUBROUTINE NavierStokes_PreSolveUpdateBoundaryConditions(solver,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error !The error string
    !Local Variables
    INTEGER(INTG) :: analyticFunctionType,boundaryConditionCheckVariable,componentIdx,componentNumberVelocity, &
      ...
      & fluidVariableType,globalDerivIndex,globalDof,I,independentDof,inputIteration,interpolationType, &
    ...
    LOGICAL :: fluidEquationsSetFound=.FALSE.,ghostNode,importDataFromFile,nodeExists, &
...
999 ERRORS("NavierStokes_PreSolveUpdateBoundaryConditions",err,error)
  ...
  SUBROUTINE NavierStokes_PreSolveALEUpdateMesh(solver,err,error,*)
    ...
    INTEGER(INTG) :: componentIdx,currentIteration,derivativeIdx,equationsSetIdx,esSpecification(3),fluidNode, &
      ...
      & laplaceNumberOfDimensions,localDOF,nodeIdx,numberOfComponents, &
      & numberOfEquationsSets,numberOfNodes,numberOfNodeDerivatives,numberOfVersions,outputIteration, &
      & pSpecification(3),solidNode,solveType,totalNumberOfNodes,versionIdx
...
999 ERRORSEXITS("NavierStokes_PostSolveOutputData",err,error)
  ...
  SUBROUTINE NavierStokes_BoundaryConditionsAnalyticCalculate(equationsSet,boundaryConditions,err,error,*)
    ...
    INTEGER(INTG) :: analyticFunctionType,boundaryConditionsCheckVariable,boundaryCount,componentIdx,derivativeIdx,dimensionIdx, &
      ...
      & localDOF,maximumNumberOfElementParameters,nodeIdx,numberOfComponents,numberOfDimensions,numberOfNodes, &
    ...
    TYPE(FieldParameterSetType), POINTER :: pressureParameterSet
...
999 ERRORSEXITS("NavierStokes_FiniteElementBoundaryIntegrate",err,error)
  ...
  SUBROUTINE NavierStokes_CalculateBoundaryFlux(equationsSet,coupledEquationsSet,iteration3D1D, &
    ...
    LOGICAL, INTENT(INOUT), TARGET :: convergedFlag !<convergence flag for 3D-0D coupling
    INTEGER(INTG), INTENT(OUT) :: err !<The error code
    TYPE(VARYING_STRING), INTENT(OUT) :: error !<The error string
    !Local Variables
...
999 ERRORSEXITS("NavierStokes_CalculateBoundaryFlux",err,error)
  ...
  SUBROUTINE NavierStokes_Couple1D0D(controlLoop,solver,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error
    !Local Variables
    INTEGER(INTG) :: boundaryConditionType,boundaryNumber,componentIdx,dependentDof,derivativeIdx, &
      & inputIteration,iteration,nodeIdx,numberOfBoundaries,numberOfGroupComputationNodes, &
    ...
    TYPE(FieldVariableType), POINTER :: uDependentVariable,u1DependentVariable,dynamicVariable, &
...
999 ERRORSEXITS("NavierStokes_Couple1D0D",err,error)
  ...
  SUBROUTINE NavierStokes_Couple3D1D(controlLoop,err,error,*)
    ...
    INTEGER(INTG) :: boundaryConditionType,boundaryNumber,boundaryType1D,componentIdx,computationNode,derivativeIdx,globalDof, &
      & inputIteration,iteration,localDof,maxIterations,nodeIdx,nodeNumber,numberOfLocalNodes1D, &
    ...
    TYPE(FieldType), POINTER :: dependentField1D,dependentField3D,independentField1D
    TYPE(FieldVariableType), POINTER :: uDependentVariable1D,u1DependentVariable1D,u2DependentVariable1D, &
...
999 ERRORSEXITS("NavierStokes_Couple3D1D",err,error)
  ...
  SUBROUTINE NavierStokes_CoupleCharacteristics(controlLoop,solver,err,error,*)
    ...
    TYPE(VARYING_STRING), INTENT(OUT) :: error
    !Local Variables
    INTEGER(INTG) :: branchNumber,componentIdx,derivativeIdx,i,inputIteration,iteration,nodeIdx, &
      & numberOfBranches,numberOfGroupComputationNodes,numberOfNodes,numberOfVersions,outputIteration, &
    ...
    LOGICAL :: branchConverged(100),boundaryNode,fluxDiverged
    LOGICAL, TARGET :: localConverged
    LOGICAL, ALLOCATABLE, TARGET :: globalConverged(:)
...
999 ERRORSEXITS("NavierStokes_PostLoop",err,error)
  ...
  SUBROUTINE NavierStokes_UpdateMultiscaleBoundary(equationsSet,boundaryConditions,timeIncrement,err,error,*)
    ...
    TYPE(FieldVariableType), POINTER :: uDependentVariable,u1DependentVariable,vDependentVariable, &
...
999 ERRORSEXITS("NavierStokes_UpdateMultiscaleBoundary",err,error)
  ...
  SUBROUTINE NavierStokes_CalculateBoundaryFlux3D0D(equationsSet,err,error,*)
    ...
    INTEGER(INTG) :: boundaryID,boundaryType,componentIdx,computationNode,coupledNodeNumber,dependentBasisType, &
      ...
      & faceNumber,gaussIdx,meshComponentNumber,nodeNumber,numberOfDimensions, &
      & numberOfFaceGauss,numberOfGroupComputationNodes,numberOfLocalElements,numberOfLocalFaces, &
    ...
    INTEGER(INTG) :: mpiIerror
#endif    
    INTEGER(INTG), TARGET :: numberOfBoundaries,numberOfGlobalBoundaries
    REAL(DP) :: boundaryValue,boundaryValueTemp,courant,elementNormal(3),faceArea,faceNormal(3),facePressure,faceTraction, &
      & faceVelocity,gaussWeight,jacobian,jacobianGaussWeight,maxCourant,mu,muScale,normalDifference,normalTolerance,p0D, &
      & pressureGauss,q0D,toleranceCourant,unitNormal(3),velocityGauss(3)
    REAL(DP), TARGET :: globalBoundaryArea(10),globalBoundaryFlux(10),globalBoundaryMeanPressure(10), &
      & globalBoundaryPressure(10),localBoundaryArea(10),localBoundaryFlux(10),localBoundaryNormalStress(10), &
      & localBoundaryPressure(10)
    LOGICAL :: boundaryFace,boundary3D0DFound(10) !<convergence flag for 3D-0D coupling
    LOGICAL, TARGET :: convergedFlag
    LOGICAL, ALLOCATABLE, TARGET :: globalConverged(:)
    TYPE(BasisType), POINTER :: dependentBasis,dependentBasis2,faceBasis
    TYPE(DecompositionType), POINTER :: decomposition3D
    TYPE(DecompositionElementType), POINTER :: decompositionElement
    ...
    TYPE(FieldInterpolationParametersType), POINTER :: dependentInterpParameters,geometricInterpParameters
    TYPE(FieldInterpolatedPointMetricsType), POINTER :: geometricInterpPointMetrics
    TYPE(FieldVariableType), POINTER :: dependentVariable3D,uMaterialsVariable,uEquationsSetVariable3D, &
    ...
    CASE(EQUATIONS_SET_MULTISCALE3D_NAVIER_STOKES_SUBTYPE, &
      ...
      IF(numberOfGroupComputationNodes>1) THEN !use mpi
        ...
        CALL MPI_ErrorCheck("MPI_Allreduce",mpiIError,err,error,*999)
#else        
        CALL MPI_ALLREDUCE(C_LOC(localBoundaryFlux),C_LOC(globalBoundaryFlux),10,MPI_DOUBLE_PRECISION,MPI_SUM, &
          & groupCommunicator,mpiIError)
        CALL MPI_ErrorCheck("MPI_ALLREDUCE",mpiIError,err,error,*999)
        CALL MPI_ALLREDUCE(C_LOC(localBoundaryArea),C_LOC(globalBoundaryArea),10,MPI_DOUBLE_PRECISION,MPI_SUM, &
          & groupCommunicator,mpiIError)
        CALL MPI_ErrorCheck("MPI_ALLREDUCE",mpiIError,err,error,*999)
        CALL MPI_ALLREDUCE(C_LOC(localBoundaryPressure),C_LOC(globalBoundaryPressure),10,MPI_DOUBLE_PRECISION,MPI_SUM, &
          & groupCommunicator,mpiIError)
        CALL MPI_ErrorCheck("MPI_ALLREDUCE",mpiIError,err,error,*999)
...
#ifdef WITH_F08_MPI
      CALL MPI_Allgather(convergedFlag,1,MPI_LOGICAL,globalConverged,1,MPI_LOGICAL,groupCommunicator,mpiIError)
      CALL MPI_ErrorCheck("MPI_A",mpiIError,err,error,*999)
#else      
